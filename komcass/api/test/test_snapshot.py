...
class KomcassApiSnapshotTest(unittest.TestCase):
    ...
    def test_get_snapshot_existing_nid(self):
        ...
        did=uuid.uuid4()
        snapshotds=ormsnapshot.SnapshotDs(nid=nid, uid=uid, wid=wid, interval_init=interval_init, interval_end=interval_end, widgetname=widgetname, creation_date=creation_date, did=did)
    ...
    def test_get_snapshots_ds_nids_existing_nids(self):
        ...
        creation_date=timeuuid.uuid1()
        did=uuid.uuid4()
        snapshot=ormsnapshot.SnapshotDs(nid=nid, uid=uid, wid=wid, interval_init=interval_init, interval_end=interval_end, widgetname=widgetname, creation_date=creation_date, did=did)
    ...
    def test_get_snapshots_dp_nids_existing_nids(self):
        ...
        creation_date=timeuuid.uuid1()
        pid=uuid.uuid4()
        snapshot=ormsnapshot.SnapshotDp(nid=nid, uid=uid, wid=wid, interval_init=interval_init, interval_end=interval_end, widgetname=widgetname, creation_date=creation_date, pid=pid)
        self.assertTrue(snapshotapi.new_snapshot(snapshot=snapshot))
