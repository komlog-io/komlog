...
d3SummaryLinegraph = {
    create: function(el, datapoints, its, ets) {
        ...
        if (datapoints.length>1) {
            ...
            if (nameLength) {
                var rectWidth=12
                if (nameLength > width - rectWidth) {
                    nameLength = width - rectWidth
                }
                var rectX = width - nameLength - rectWidth - 10
                ...
                captionBox.append("rect")
                    .attr("x", rectX - 10 )
                    .attr("y", 10 )
                    .attr("width", rectWidth + 15 + nameLength)
                ...
                nameCaption.append("text")
                    ...
                    .style('fill','#444')
                    .text(function (d) {return d.datapointname})
...
d3SummaryDatasource = {
    ...
    update: function() {
        ...
        var dsLines = this.datasource.content.split('\n')
        var margin = {top: 0, right: 0, bottom: 40, left: 0};
        if (this.expanded == true) {
        ...
        var width=d3.select(this.el).node().getBoundingClientRect().width-margin.left
        var dateFormat = d3.time.format("%Y/%m/%d - %H:%M:%S")
        date=new Date(this.ts*1000)
        dateText=dateFormat(date)
        dateWidth=dateText.length*8
        dateHeight=20
        dateX=width/2-dateWidth/2
        ...
        svg.append("rect")
            ...
            .attr("height", height + margin.top + margin.bottom)
            .on("mouseenter", function() {
                d3.select(this.el).select('.date-tooltip')
                    .transition()
                    .duration(300)
                    .style('opacity', 0);
             }.bind(this))
