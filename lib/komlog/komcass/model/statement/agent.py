...
STATEMENTS={
    ...
    10300:'select sid,aid,uid,pv,last_update,imc_address from mst_agent_session where sid=?',
    ...
    15300:'insert into mst_agent_session (sid,aid,uid,pv,imc_address,last_update) values (?,?,?,?,?,?)',
    ...
    19300:'update mst_agent_session set aid=?,uid=?,pv=?,imc_address=?,last_update=? where sid=? if last_update <= ?',
}
