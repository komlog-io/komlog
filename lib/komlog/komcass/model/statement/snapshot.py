...
STATEMENTS={
    120000:'select nid,uid,wid,type,interval_init,interval_end,widgetname,creation_date from mst_snapshot where nid=?',
    120001:'select nid,uid,wid,type,interval_init,interval_end,widgetname,creation_date from mst_snapshot where uid=?',
    120002:'select nid,uid,wid,type,interval_init,interval_end,widgetname,creation_date from mst_snapshot where wid=?',
    ...
    120100:'select nid,did,datasource_config,datapoints_config from mst_snapshot_ds where nid=?',
    120200:'select nid,pid,datapoint_config from mst_snapshot_dp where nid=?',
    120300:'select nid,datapoints,colors from mst_snapshot_histogram where nid=?',
    120400:'select nid,datapoints,colors from mst_snapshot_linegraph where nid=?',
    120500:'select nid,datapoints,colors from mst_snapshot_table where nid=?',
    120600:'select nid,active_visualization,datapoints,datapoints_config from mst_snapshot_multidp where nid=?',
    125000:'insert into mst_snapshot (nid,uid,wid,type,interval_init,interval_end,widgetname,creation_date) values (?,?,?,?,?,?,?,?)',
    125001:'insert into mst_snapshot (nid,uid,wid,type,interval_init,interval_end,widgetname,creation_date) values (?,?,?,?,?,?,?,?) if not exists',
