...
class InterfaceImcModelMessagesTest(unittest.TestCase):
    ...
    def test_GenerateDTreeMessage_failure_invalid_did(self):
        ''' GenerateDTreeMessage creation should fail if did is invalid '''
        dids=[None, 23423, 2323.2342, 'User/name',{'a','dict'},['a','list'],('a','tuple'),'userñame',json.dumps('username'), 'user\nname','user\tname', timeuuid.uuid1()]
        for did in dids:
            with self.assertRaises(exceptions.BadParametersException) as cm:
                messages.GenerateDTreeMessage(did=did)
            self.assertEqual(cm.exception.error, Errors.E_IIMM_GDTREE_IDID)
    ...
    def test_GenerateDTreeMessage_failure_load_from_serialization_invalid_hex_pid(self):
        ...
        did=uuid.uuid4()
        msg='|'.join((messages.GenerateDTreeMessage._type_.value,'did.hex'))
        with self.assertRaises(exceptions.BadParametersException) as cm:
            messages.GenerateDTreeMessage.load_from_serialization(msg)
        self.assertEqual(cm.exception.error, Errors.E_IIMM_GDTREE_IHDID)
    ...
    def test_DataIntervalRequestMessage_failure_invalid_count(self):
        ''' DataIntervalRequestMessage creation should fail if count is invalid '''
        counts=[2323.2342, -1, 'Username',{'a','dict'},['a','list'],('a','tuple'),'userñame',uuid.uuid4(), uuid.uuid1().hex, json.dumps('username'), 'user\nname','user\tname']
        uri={'uri':'valid.uri','type':'type','id':uuid.uuid4()}
        sid=uuid.uuid4()
        ii=timeuuid.uuid1()
        ie=timeuuid.uuid1()
        for count in counts:
            with self.assertRaises(exceptions.BadParametersException) as cm:
                messages.DataIntervalRequestMessage(sid=sid, uri=uri, ii=ii, ie=ie, count=count)
            self.assertEqual(cm.exception.error, Errors.E_IIMM_DIRM_ICOUNT)
    ...
    def test_DataIntervalRequestMessage_failure_invalid_irt(self):
        ...
        for irt in irts:
            with self.assertRaises(exceptions.BadParametersException) as cm:
                messages.DataIntervalRequestMessage(sid=sid, uri=uri, ii=ii, ie=ie, count=count, irt=irt)
    ...
    def test_DataIntervalRequestMessage_success_irt_is_not_none(self):
        ''' DataIntervalRequestMessage creation should succeed if irt is a valid message sequence '''
        uri={'uri':'valid.uri','type':'type','id':uuid.uuid4().hex}
        obj_uri={'uri':'valid.uri','type':'type','id':uuid.UUID(uri['id'])}
        sid=uuid.uuid4()
        ii=timeuuid.uuid1()
    ...
    def test_DataIntervalRequestMessage_failure_load_from_serialization_invalid_json_count(self):
        ''' DataIntervalRequestMessage creation should fail if we pass a string with invalid count '''
        sid=uuid.uuid4()
        ii=uuid.uuid1()
        ie=uuid.uuid1()
        uri={'uri':'uri','type':'ds','id':uuid.uuid4().hex}
    ...
    def test_DataIntervalRequestMessage_failure_load_from_serialization_invalid_json_irt(self):
        ''' DataIntervalRequestMessage creation should fail if we pass a string with invalid json irt '''
        sid=uuid.uuid4()
        ii=uuid.uuid1()
        ie=uuid.uuid1()
        uri={'uri':'uri','type':'ds','id':uuid.uuid4().hex}
    ...
    def test_DataIntervalRequestMessage_to_serialization_success(self):
        ...
        self.assertEqual(obj2.count,count)
        self.assertEqual(obj2.irt,irt)
        self.assertEqual(obj._type_, messages.Messages.DATA_INTERVAL_REQUEST_MESSAGE)
    ...
    def test_UpdateDatasourceFeaturesMessage_failure_invalid_did(self):
        ''' UpdateDatasourceFeaturesMessage creation should fail if did is invalid '''
        dids=[None, 23423, 2323.2342, 'User/name',{'a','dict'},['a','list'],('a','tuple'),'userñame',json.dumps('username'), 'user\nname','user\tname', timeuuid.uuid1()]
        for did in dids:
            with self.assertRaises(exceptions.BadParametersException) as cm:
    ...
    def test_UpdateDatasourceFeaturesMessage_success_load_from_serialization(self):
        ...
        did=uuid.uuid4()
        msg='|'.join((messages.UpdateDatasourceFeaturesMessage._type_.value,did.hex))
        obj=messages.UpdateDatasourceFeaturesMessage.load_from_serialization(msg)
        self.assertEqual(obj.did, did)
        self.assertEqual(obj._type_, messages.Messages.UPDATE_DATASOURCE_FEATURES_MESSAGE)
        self.assertTrue(isinstance(obj, messages.UpdateDatasourceFeaturesMessage))
        self.assertTrue(isinstance(obj, messages.IMCMessage))

    def test_UpdateDatasourceFeaturesMessage_success_load_from_serialization_base_class(self):
        '''  UpdateDatasourceFeaturesMessage creation should succeed calling the classmethod load_from_serialization from the base class '''
        did=uuid.uuid4()
        msg='|'.join((messages.UpdateDatasourceFeaturesMessage._type_.value,did.hex))
        obj=messages.IMCMessage.load_from_serialization(msg)
        self.assertEqual(obj.did, did)
    ...
    def test_UpdateDatasourceFeaturesMessage_to_serialization_success(self):
        ''' UpdateDatasourceFeaturesMessage.to_serialization should succeed '''
        did=uuid.uuid4()
        msg='|'.join((messages.UpdateDatasourceFeaturesMessage._type_.value, did.hex))
        obj=messages.IMCMessage.load_from_serialization(msg)
        self.assertEqual(obj.did, did)
