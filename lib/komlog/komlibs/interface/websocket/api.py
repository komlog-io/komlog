...
def process_message(passport, message):
    if not isinstance(passport, Passport):
        return responsev1.Response(status=statusv1.INTERNAL_ERROR, reason='Ups... Internal error', error=errorsv1.E_IWSA_PM_IPSP.value)
    if not args.is_valid_dict(message)\
        ...
        return responsev1.Response(status=statusv1.PROTOCOL_ERROR, reason='Send a valid version and action', error=errorsv1.E_IWSA_PM_IVA.value)
    if message['v']==1:
        return apiv1.process_message(passport, message)
    else:
        return responsev1.Response(status=statusv1.PROTOCOL_ERROR, reason='Unsupported protocol version', error=errorsv1.E_IWSA_PM_UPV.value)
