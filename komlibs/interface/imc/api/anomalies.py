...
def process_message_MISSINGDP(message):
    ...
    if len(dp_classified['doubts'])>0:
        ...
        response.add_msg_originated(messages.UserEventMessage(uid=datasource['uid'], event_type=eventstypes.USER_INTERVENTION_DATAPOINT_IDENTIFICATION, parameters={'did':did.hex,'date':date.hex,'doubts':[pid.hex for pid in dp_classified['doubts']],'discarded':[pid.hex for pid in dp_classified['discarded']]}))
